<#
.SYNOPSIS
    Guided mass recovery workflow for EHR GCP instances via Rubrik Security Cloud.

.DESCRIPTION
    Invoke-EhrGcpRecovery.ps1 provides an interactive, step-by-step recovery workflow
    for EHR environments hosted on GCP and protected by Rubrik Security Cloud (RSC).
...
.PARAMETER MbfConfigXml
    Path to the DPAPI-encrypted credential XML created by New-EhrCredentials.ps1.
    ...
    Default: C:\ProgramData\Rubrik\EHRCreds.xml
...
.PARAMETER GcpProjectId
    ...
    Can be embedded in -MbfConfigXml (set via New-EhrCredentials.ps1).
    Use -ListProjects on EHRMasterScript.ps1 to find available project UUIDs.
...
.PARAMETER TargetSubnetName
    [Export only] GCP subnet name for recovered instances (e.g. "ehr-uscentral1-a").
    Prompted interactively if not provided.
...
.EXAMPLE
    # Export (clone) all EHR VMs in a project to a DR environment.
    .\Invoke-EhrGcpRecovery.ps1 `
        -MbfConfigXml "C:\ProgramData\Rubrik\EHRCreds.xml" `
...
.NOTES
    ...
    - Run New-EhrCredentials.ps1 once to create the encrypted credential XML.
...
param (
    ...
    [string]$MbfConfigXml = "C:\ProgramData\Rubrik\EHRCreds.xml",
    ...
    [Parameter()]
    [string]$LogPath = "C:\ProgramData\Rubrik\Logs\EHRRecovery.log"
)
...
try {
    ...
    Write-Log "   Rubrik | EHR GCP Recovery" -ForegroundColor Cyan
    Write-Log "========================================================" -ForegroundColor Cyan
    Write-Log ""
    ...
    if ($MbfConfigXml -and (Test-Path $MbfConfigXml)) {
        ...
        if (-not $mbfStore.RscClientId) {
            Throw "RSC credentials not found in '$MbfConfigXml'. Re-run New-EhrCredentials.ps1 to embed RSC service account details."
        }
    ...
    if ($RecoveryType -eq "Restore") {
        ...
        if (-not $TargetSubnetName) {
            Write-Log ""
            $TargetSubnetName = (Read-Host "    Target Subnet Name (e.g. ehr-uscentral1-a)").Trim().Trim('"').Trim("'")
        }
