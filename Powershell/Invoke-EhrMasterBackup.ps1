<#
.SYNOPSIS
    Orchestrates EHR GCP backup: MBF quiesce, Rubrik snapshot, MBF unquiesce.

.DESCRIPTION
    EHRMasterScript.ps1 automates the full crash-consistent backup cycle for
    EHR environments hosted on GCP and protected by Rubrik Security Cloud (RSC).

    WORKFLOW (RunBackup mode — default)
    ------------------------------------
    Step 1  Authenticate to RSC using a service account.
    Step 2  Run MBF Census to discover active EHR servers and resolve them to
            GCP VM IDs in the RSC inventory.
    Step 3  Quiesce EHR via mbf.exe (freeze I/O for a consistent snapshot).
    ...
    8     EHR servers are busy; retry recommended (-Force retries automatically).
...
.PARAMETER MbfConfigXml
    Path to the DPAPI-encrypted XML credential file created by New-EhrCredentials.ps1.
    ...
    Default path if you used New-EhrCredentials.ps1 without -OutputPath:
      C:\ProgramData\Rubrik\EHRCreds.xml
...
.PARAMETER ServiceAccountJson
    ...
    Use New-EhrCredentials.ps1 to embed the service account into the encrypted
...
.PARAMETER ListSlas
    ...
    Use the UUID as -RetentionSlaId (or embed it via New-EhrCredentials.ps1).
...
.PARAMETER LogPath
    ...
    Default: C:\ProgramData\Rubrik\Logs\EHRBackup.log

.EXAMPLE
    # Standard scheduled-task invocation using the encrypted credential file.
    # Run New-EhrCredentials.ps1 once as the service account to create the XML.
    .\EHRMasterScript.ps1 -MbfConfigXml "C:\ProgramData\Rubrik\EHRCreds.xml" -EnableLogging

.EXAMPLE
    # Discover available SLA domains to find the correct RetentionSlaId.
    .\EHRMasterScript.ps1 -MbfConfigXml "C:\ProgramData\Rubrik\EHRCreds.xml" -ListSlas

.EXAMPLE
    # Discover available GCP projects to find the correct GcpProjectId.
    .\EHRMasterScript.ps1 -MbfConfigXml "C:\ProgramData\Rubrik\EHRCreds.xml" -ListProjects

.EXAMPLE
    # Validate MBF connectivity without connecting to RSC or taking any snapshots.
    .\EHRMasterScript.ps1 -MbfConfigXml "C:\ProgramData\Rubrik\EHRCreds.xml" -CensusOnly
...
.EXAMPLE
    ...
    .\EHRMasterScript.ps1 `
        -MbfConfigXml "C:\ProgramData\Rubrik\EHRCreds.xml" `
...
.NOTES
    ...
    - Run New-EhrCredentials.ps1 once before scheduling this script.
    ...
    RELATED SCRIPTS
    ---------------
    New-EhrCredentials.ps1 — one-time setup wizard that creates the encrypted
...
param (
    ...
    # RSC credentials (generated by New-EhrCredentials.ps1).
    ...
    # Run New-EhrCredentials.ps1 once to create this file. It is DPAPI-encrypted
    ...
    [Parameter()]
    [string]$LogPath = "C:\ProgramData\Rubrik\Logs\EHRBackup.log"
)
...
try {
    ...
    if ($MbfConfigXml) {
        if (-not (Test-Path $MbfConfigXml)) {
            Throw "Config XML not found at '$MbfConfigXml'. Run New-EhrCredentials.ps1 to create it."
        }
    ...
    if ($PSCmdlet.ParameterSetName -eq "RunBackup" -and [string]::IsNullOrEmpty($RetentionSlaId)) {
        Throw "RetentionSlaId not provided. Supply -RetentionSlaId or embed it in -MbfConfigXml via New-EhrCredentials.ps1."
    }
    ...
    Write-Log ">>> Step 2: Running EHR Census to identify targets..." -ForegroundColor Cyan
    ...
    if ($censusResult.Luns.Count -eq 0) {
        Throw "EHR Census returned no LUNs/Servers. Check MBF configuration or connectivity."
    }

    # Extract unique server names from Census (Case-insensitive)
    $ehrHosts = $censusResult.Luns.Server | Select-Object -Unique
    Write-Log "    Census identified $($ehrHosts.Count) unique host(s): $($ehrHosts -join ', ')" -ForegroundColor Gray
    ...
    $targetWorkloads = $inventory | Where-Object { 
        $ehrHosts -contains $_.nativeName 
    }

    $foundHosts = $targetWorkloads.nativeName
    $missingHosts = $ehrHosts | Where-Object { $foundHosts -notcontains $_ }
    
    if ($missingHosts) {
        Write-Log "The following EHR servers were NOT found in the Rubrik Inventory:" -Level Warning
        Write-Log ($missingHosts -join ", ") -Level Warning
    }

    if ($targetWorkloads.Count -eq 0) {
        Throw "No matching GCE Instances found in RSC Inventory corresponding to EHR Census."
    }
    ...
    # 3. QUIESCE MEDITECH
    Write-Log ">>> Step 3: Quiescing EHR..." -ForegroundColor Yellow
    ...
    Write-Log ">>> Step 5: Unquiescing EHR..." -ForegroundColor Yellow
    ...
    if ($isQuiesced) {
        Write-Log "EMERGENCY: Script exited while EHR was still Quiesced." -Level Warning
