...
.SYNOPSIS
    ...
    Windows Scheduled Task for EHRMasterScript.ps1.
...
.DESCRIPTION
    ...
    that runs EHRMasterScript.ps1 on a daily schedule or every X hours.
...
.PARAMETER OutputPath
    ...
    Defaults to C:\ProgramData\Rubrik\EHRCreds.xml.
    The directory is created automatically if it does not exist.
    This path must match the -MbfConfigXml value used with EHRMasterScript.ps1.
...
.EXAMPLE
    ...
    .\New-EhrCredentials.ps1 -OutputPath "D:\Rubrik\EHRCreds.xml"

    # Reference the same path when running the main script:
    .\EHRMasterScript.ps1 -MbfConfigXml "D:\Rubrik\EHRCreds.xml" -EnableLogging
...
.NOTES
    ...
    - Run as the Windows account that Task Scheduler will use to execute
      EHRMasterScript.ps1. DPAPI ties the encrypted output file to this specific
    ...
    - Consider restricting the XML file's ACL after setup so only the service account
      can read it:
        icacls "C:\ProgramData\Rubrik\EHRCreds.xml" /inheritance:r /grant:r "${env:USERNAME}:(R)"
...
param (
    [Parameter()]
    [string]$OutputPath = "C:\ProgramData\Rubrik\EHRCreds.xml"
)

$ErrorActionPreference = "Stop"

Write-Host ""
Write-Host "=== EHR Credential Setup ===" -ForegroundColor Cyan
...
Write-Host "           will use to execute EHRMasterScript.ps1." -ForegroundColor Magenta
Write-Host ""
...
Write-Host "        Run EHRMasterScript.ps1 with -ListSlas to find this value." -ForegroundColor Gray
Write-Host ""
...
Write-Host "        Run EHRMasterScript.ps1 with -ListProjects to find this value." -ForegroundColor Gray
...
if ($createTask -match '^[Yy]') {
    ...
    # --- SCRIPT PATH ---
    Write-Host ""
    $masterScriptPath = (Read-Host "Full path to EHRMasterScript.ps1").Trim().Trim('"').Trim("'")
    ...
    if ([string]::IsNullOrWhiteSpace($masterScriptPath) -or -not (Test-Path $masterScriptPath)) {
        ...
        # --- TASK NAME ---
        Write-Host ""
        $taskNameInput = (Read-Host "Task name (press Enter for default: 'Rubrik - EHR Backup')").Trim().Trim('"').Trim("'")
        $taskName = if ([string]::IsNullOrWhiteSpace($taskNameInput)) { "Rubrik - EHR Backup" } else { $taskNameInput }
        ...
        try {
            ...
            Write-Host "  Log file   : C:\ProgramData\Rubrik\Logs\EHRBackup.log" -ForegroundColor Gray
            Write-Host ""
...
Write-Host "      -File `"<path to EHRMasterScript.ps1>`" ``" -ForegroundColor Gray
...
Write-Host "  .\EHRMasterScript.ps1 -MbfConfigXml `"$OutputPath`" -ListSlas" -ForegroundColor Gray
Write-Host ""
Write-Host "  # List GCP projects:" -ForegroundColor Gray
Write-Host "  .\EHRMasterScript.ps1 -MbfConfigXml `"$OutputPath`" -ListProjects" -ForegroundColor Gray
Write-Host ""
Write-Host "  # Run a Census only (no backup, no RSC connection):" -ForegroundColor Gray
Write-Host "  .\EHRMasterScript.ps1 -MbfConfigXml `"$OutputPath`" -CensusOnly" -ForegroundColor Gray
Write-Host ""
